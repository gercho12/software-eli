...
const Proveedores = () => {
  ...
  const handleGuardarEdicion = async () => {
    ...
    try {
      await axios.put(`http://localhost:8800/proveedor/${seleccionadoProveedor.cuit}`, datosActualizados);
  ...
  const handleEliminar = async (cuitEliminar) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:8800/proveedor/eliminar/${cuitEliminar}`);
      console.log(response.data); // Verifica la respuesta del servidor en la consola
      const result = await axios.get('http://localhost:8800/proveedor');
      setProveedores(result.data);
    } catch (error) {
